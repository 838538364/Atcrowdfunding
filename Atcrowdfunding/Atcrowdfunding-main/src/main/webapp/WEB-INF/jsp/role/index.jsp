            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <button type="button" class="btn btn-primary" style="float:right;"
                            onclick="window.location.href='${pageContext.request.contextPath}/role/toAdd.htm'"><i class="glyphicon glyphicon-plus"></i> 新增
                    </button>
                    <br>
                    <hr style="clear:both;">
...
<script type="text/javascript">
    ...
    function queryPageUser(pageno) {
        ...
        $.ajax({
            type: "POST",
            data: jsonObj,
            url: "${pageContext.request.contextPath}/role/toIndex.do",
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    $.each(data, function (index, role) {
                        ...
                        content += '  <td><input type="checkbox" id="' + role.id + '" name="' + role.name + '"></td>';

                        content += '  <td>' + role.name + '</td>';

                        content += '  <td>';

                        content += '          <button type="button" onclick="window.location.href=\'${pageContext.request.contextPath}/role/toAssignPermission.htm?roleid=' + role.id + '\'" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';

                        content += '          <button type="button"  onclick="window.location.href=\'${pageContext.request.contextPath}/role/toUpdate.htm?id=' + role.id + '\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
    ...
    function deleteRole(id, name) {
        ...
        layer.confirm("是否删除[" + name + "]用户？", {icon: 3, title: '提示'}, function (cindex) {
            ...
            $.ajax({
                ...
                url: "${pageContext.request.contextPath}/role/doDelete.do",
                beforeSend: function () {
                    return true;
                },
                success: function (result) {
    ...
    $("#deleteBatchBtn").click(function () {
        ...
        layer.confirm("是否删除这些用户？", {icon: 3, title: '提示'}, function (cindex) {
            ...
            $.ajax({
                type: "POST",
                data: dataObj,
                url: "${pageContext.request.contextPath}/role/doDeleteRoleBatch.do",
                beforeSend: function () {
                    return true;
                },
                success: function (result) {
